...
const radioGroups = document.querySelectorAll(
    '[by-quiz-element="button-answer-group"]'
)
const buttonSubmit = document.querySelectorAll(
    '[by-quiz-element="button-submit"]'
)
const buttonRedirectResults = document.querySelector(
    '[by-quiz-element="button-redirect-results"]'
)
const buttonRedirectThanks = document.querySelector(
    '[by-quiz-element="button-redirect-thankyou"]'
)

const inputElement = document.querySelector('[by-quiz-element="input-name"]')
const inputResultsUrlElement = document.querySelector(
    '[by-resultsurl-element="input"]'
)
const mirrorElement = document.querySelector(
    '[by-quiz-element="input-name-mirror"]'
)
...
function calculateCategoryTotals() {
    ...
    for (const category in categoryScores) {
        const total = Object.values(categoryScores[category]).reduce(
            (acc, score) => acc + score,
            0
        )
...
function updateUrl() {
    ...
    const queryParams = [
        ...Object.entries(categoryTotals).map(
            ([category, total]) => `Score${category}=${total}`
        ),
        `Name=${nameValue}`,
    ].join('&')
    ...
    if (buttonRedirectResults) {
        buttonRedirectResults.href = finalUrl
    }
}
...
radioGroups.forEach((answerGroup) => {
    ...
    observer.observe(
        answerGroup.querySelector('label.quiz_answer-radio-field'),
        config
    )
})
...
const progressIndicator = document.querySelector(
    '[by-quizstep-progress="indicator"]'
)
const progressPercent = document.querySelector(
    '[by-quizstep-progress="percent"]'
)
...
const countdownElement = document.querySelector(
    '[by-quiz-element="redirect-countdown"]'
)

if (quizForm && buttonRedirectResults && countdownElement) {
    quizForm.addEventListener('submit', function (event) {
        ...
        const countdownInterval = setInterval(function () {
            ...
            if (countdownValue === 0) {
                // Trigger the click event on the redirect button element
                // buttonRedirectResults.click()
                buttonRedirectThanks.click()
    ...
    console.error(
        'Quiz form, buttonRedirectResults, or countdown element not found.'
    )
}
